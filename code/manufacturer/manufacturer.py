...
@manufacturer_bp.route("/mowerpagemf.html")
def mower():
    #verifies that logged in user is a manufacturer
    role = session["role"]
    if role != "manufacturer":
    ...
    return render_template("mowerpagemf.html", title = "Mower Page")



@manufacturer_bp.route("/requestmf.html", methods = ["GET", "POST"])
def requestmf():
    #verifies that logged in user is a manufacturer
    role = session["role"]
    if role != "manufacturer":
    ...
    return render_template("requestmf.html", title = "Request Page", data = request_data,)


@manufacturer_bp.route("/requestinfomf", methods = ["GET", "POST"])
def requestinfo():
    #verifies that logged in user is a manufacturer
    ...
    if "request_id" in session:
        ...
        return render_template("requestinfomf.html", request=request, providers = providers, products = products)
    else:
...
def remove(id):
  #Finds data from database and removes it 
    ...
    return redirect(url_for("manufacturer.requestmf"))
...
@manufacturer_bp.route("/customerinfomf", methods = ["GET", "POST"])
def customerinfo():
    #verifies that logged in user is a manufacturer
    role = session["role"]
    if role != "manufacturer":
    ...
    if "customer_id" in session:
        ...
        return render_template("customerinfomf.html", customer=customer)
    else:
        return redirect(url_for("manufacturer.manufacturer"))
...
def infoServiceProvider():
    ...
    return render_template("infoServiceProvidermf.html", title = "Assigned Areas", assignedAreas = assignedAreas)

@manufacturer_bp.route("/areainfomf.html")
def areainfo():
    #verifies that logged in user is a manufacturer
    role = session["role"]
    if role != "manufacturer":
    ...
    return render_template("areainfomf.html", title = "Area Info")

@manufacturer_bp.route("/productlistmf.html")
def productlist():
    #verifies that logged in user is a manufacturer
    role = session["role"]
    if role != "manufacturer":
    ...
    return render_template("productlistmf.html", title = "Product List", product=product_data)
...
@manufacturer_bp.route("/addproductmf.html", methods=("GET", "POST"))
def addproduct():
    #verifies that logged in user is a manufacturer
    role = session["role"]
    if role != "manufacturer":
    ...
    if request.method == "POST":
        ...
        form = ProductForm()
    return render_template("addproductmf.html", title = "Add Product", form = form)
